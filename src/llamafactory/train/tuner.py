...
def export_model(args: Optional[Dict[str, Any]] = None) -> None:
    ...
    # The rotated model has different architecture compared to the baseline model, need to save state dict ,safe_serialization=False
    if model_args.rotate_down_proj or (model_args.rotate and 'noreplace' not in model_args.export_dir):
        if not os.path.exists(model_args.export_dir):
            os.makedirs(model_args.export_dir)
            print(f"Directory '{model_args.export_dir}' created.")
        torch.save(model.state_dict(), os.path.join(model_args.export_dir, 'rotate_model.bin'))
